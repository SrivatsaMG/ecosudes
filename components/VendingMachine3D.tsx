...
import { Environment, OrbitControls, PerspectiveCamera, Text, RoundedBox, useTexture } from "@react-three/drei"
...
function Machine() {
  ...
  // Load logo texture
  const logoTexture = useTexture("/ecosuds-logo.png")

  return (
    <group position={[0, -1, 0]}>
      {/* Main body with rounded corners */}
      <RoundedBox args={[2, 3.5, 1]} radius={0.1} smoothness={4} position={[0, 1, 0]}>
        <meshStandardMaterial color="#f5f5f5" metalness={0.7} roughness={0.2} envMapIntensity={1} />
      </RoundedBox>

      {/* Logo on the top of the machine */}
      <mesh position={[0, 2.76, 0.51]} rotation={[-Math.PI / 2, 0, 0]}>
...
export default function VendingMachine3D({ className }: { className?: string }) {
  return (
    <div className={className}>
      <Canvas>
        ...
        <spotLight position={[5, 5, 5]} intensity={1} castShadow angle={0.5} penumbra={1} />
        <Machine />
